...
- Add `equivalentCommands: ["..."]` to a tool's config for the raw CLI it approximates — defineTool folds it into MCP `_meta` and the generated docs/tools.md; new groups must be added to the `GROUPS` table in src/registry.ts (NOT index.ts directly), giving each a README category. `npm run docs:tools` regenerates docs/tools.md AND the README generated regions (`<!-- BEGIN/END GENERATED: tools|which-tool -->` — grouped tool tables + the "Which tool?" intent table); run it after touching a tool's title/description, `equivalentCommands`, the `GROUPS` table, or guidance `INTENTS`, or the registry guard test fails. The README table blurbs are the first sentence of each tool's description — keep that sentence self-contained
...
## Commands
Build: npm run build | Dev: npm run dev | Test: npm test | Lint: npm run lint | Typecheck: npm run typecheck | Tool reference: npm run docs:tools (regenerates docs/tools.md + README generated regions; `-- --check` fails if stale)

## Prerequisites
Node.js >= 20 always required. Per-category: rg (ripgrep), git, kubectl, terraform, helm, argocd, jq, yq (mikefarah), dotnet, liquibase, ruff, mypy, pytest. Tools error gracefully if CLI missing.
