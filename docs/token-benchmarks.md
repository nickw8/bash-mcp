...
| Frequency-weighted total | **40%** | Weighted by an illustrative session mix (read/diff/log/diagnose dominate, bulk infra listings are rare — see `weight` in the manifest). |
...
**frequency-weighted 40%** already prices these in at their real call mix — `rg` alone carries
