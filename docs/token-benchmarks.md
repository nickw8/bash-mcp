...
| `liquibase updateSQL` (→ `liquibase_update_sql`) | 1154 | 262 | 77% |
...
| `liquibase status` (→ `liquibase_status`) | 240 | 168 | 30% |
| `liquibase validate` (→ `liquibase_validate`) | 227 | 161 | 29% |
| `tree` (→ `tree`) | 81 | 59 | 27% |
...
| **TOTAL (token-weighted)** | 6829 | 3018 | **56%** |
...
| --- | ---: | --- |
| Token-weighted total | **56%** | `(Σraw − Σstruct)/Σraw` — dominated by the few large samples (plan, describe, outline). |
| Median per-command reduction | **34%** | Robust central tendency across the 35 commands; ignores sample size. |
| Frequency-weighted total | **52%** | Weighted by an illustrative session mix (read/diff/log/diagnose dominate, bulk infra listings are rare — see `weight` in the manifest). |
