...
export function registerTerraformTools(server: McpServer) {
  ...
  server.registerTool("tf_state_list", {
    ...
    const result = await exec("terraform", ["state", "list"], { cwd, timeout: 30_000 });
  ...
  server.registerTool("tf_show", {
    ...
    const result = await execJson<TfShowState>(
      "terraform", ["show", "-json"], { cwd, timeout: 60_000 },
    );
    ...
    const childResources = (result.data?.values?.root_module?.child_modules ?? [])
  ...
  server.registerTool("tf_plan_summary", {
    ...
    for (const line of result.stdout.split("\n").filter(Boolean)) {
      try {
        ...
        if (msg.type === "planned_change" || msg.type === "resource_drift") {
          const c = msg.change ?? {};
          const action = Array.isArray(c.action) ? c.action.join(",") : (c.action ?? "unknown");
          changes.push({
            action,
    ...
    for (const line of result.stderr.split("\n")) {
      const match = line.match(/(\d+) to add, (\d+) to change, (\d+) to destroy/);
      if (match) {
  ...
  // ── terraform workspace ─────────────────────────────────────────────
  server.registerTool("tf_workspaces", {
 ...
 */
function pickImportantAttributes(type: string, values: Record<string, unknown>): Record<string, string> {
  const important: Record<string, string[]> = {
    aws_instance: ["id", "instance_type", "ami", "private_ip", "public_ip", "tags"],
    aws_security_group: ["id", "name", "vpc_id"],
    aws_db_instance: ["id", "engine", "engine_version", "instance_class", "identifier"],
  ...
  for (const key of keys) {
    if (values[key] !== undefined) {
      result[key] = typeof values[key] === "object" ? JSON.stringify(values[key]) : String(values[key]);
    }
  }
  return result;
}
