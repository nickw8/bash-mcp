...
export function registerFileTools(server: McpServer) {
  server.registerTool("cat", {
    ...
    inputSchema: {
      ...
      maxLines: z.number().optional().default(200).describe("Max lines to return (default 200, use 0 for unlimited)"),
      lineNumbers: z.boolean().optional().describe("Prepend line numbers to each line (useful before editing)"),
    },
    outputSchema: {
      path: z.string(),
      totalLines: z.number(),
    ...
    const empty: { path: string; totalLines: number; size: number; content: string; range: [number, number]; truncated: boolean } =
      { path, totalLines: 0, size: 0, content: "", range: [0, 0], truncated: false };

    // Get file size in bytes
    const sizeResult = await exec("stat", IS_MACOS ? ["-f", "%z", path] : ["--printf=%s", path]);
    if (sizeResult.exitCode !== 0) {
      return err(sizeResult.stderr || `Cannot stat file: ${path}`, empty);
    }
    ...
    const totalLines = parseInt(wcResult.stdout.trim().split(/\s+/)[0] ?? "0", 10);
    ...
    if (startLine !== undefined || endLine !== undefined) {
      ...
      rangeEnd = endLine !== undefined ? Math.min(endLine, totalLines) : totalLines;

      // Apply maxLines cap within the range
      if (maxLines !== undefined && maxLines > 0 && (rangeEnd - rangeStart + 1) > maxLines) {
      ...
      const sedResult = await exec("sed", ["-n", `${rangeStart},${rangeEnd}p`, path]);
    ...
    if (lineNumbers) {
      ...
      content = lines.map((line, i) => `${String(startNum + i).padStart(6)}\t${line}`).join("\n");
    }
  ...
  server.registerTool("outline", {
    ...
    const empty = { path, language: "unknown", totalLines: 0, outlineLines: 0, symbols: 0, outline: "" };
