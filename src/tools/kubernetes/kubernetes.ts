...
export function registerKubernetesTools(server: McpServer) {
  ...
  server.registerTool("kube_get", {
    ...
    inputSchema: {
      resource: z.string().describe("Resource type (e.g. pods, deployments, services, nodes)"),
      namespace: z.string().optional().describe("Namespace (omit for all-namespaces or cluster-scoped)"),
    ...
    const result = await execJson<KubeList>(
      "kubectl", args, { timeout: 15_000 },
    );
    ...
    const rawItems = result.data?.kind === "List"
      ? (result.data.items ?? [])
      : [result.data];

    const items = rawItems.filter(Boolean).map((item) => summarizeResource(item!));
    return ok({ items, count: items.length, resource });
  });

  // ── kubectl logs ────────────────────────────────────────────────────
  server.registerTool("kube_logs", {
    title: "Kubectl logs",
    description:
    ...
    inputSchema: {
      ...
      tail: z.number().optional().default(100).describe("Number of lines (default 100)"),
    ...
    const args = ["logs", pod, "-n", namespace ?? "default", "--timestamps", `--tail=${tail ?? 100}`];
  ...
  // ── kubectl contexts ────────────────────────────────────────────────
  server.registerTool("kube_contexts", {
    title: "Kubectl contexts",
    description: "List available kubectl contexts with current context marked.",
    inputSchema: {},
    outputSchema: {
      current: z.string(),
...
function summarizeResource(item: KubeResource) {
  ...
  if (spec.replicas !== undefined) extra.replicas = `${status.readyReplicas ?? 0}/${spec.replicas}`;
  if (spec.type) extra.type = spec.type;
  if (spec.clusterIP) extra.clusterIP = spec.clusterIP;
  if (status.containerStatuses) {
    const restarts = status.containerStatuses.reduce((s, c) => s + c.restartCount, 0);
...
function inferStatus(conditions?: Array<{ type: string; status: string }>): string {
