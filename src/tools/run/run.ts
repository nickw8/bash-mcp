...
export function registerRunTools(server: McpServer) {
  server.registerTool("run", {
    ...
    inputSchema: {
      ...
      args: z.array(z.string()).optional().default([]).describe("Command arguments (e.g. ['test'])"),
      cwd: z.string().optional().describe("Working directory"),
      timeout: z.number().optional().default(30000).describe("Timeout in ms"),
      maxLines: z.number().optional().default(50).describe("Max stdout lines to keep (last N). 0 = unlimited."),
    },
    outputSchema: {
      exitCode: z.number(),
      stdout: z.string(),
      ...
      truncated: z.boolean(),
      elapsed: z.number().describe("Wall-clock execution time in milliseconds"),
    },
