...
| **Text block vs structuredContent** | Every response carries both. `structuredContent` is the complete typed payload and **the artifact the agent is charged tokens for** — a client that understands it renders it and ignores the text block. The text block is a *Summary line*, not a second copy. They are deliberately not the same. See [ADR-0009](docs/adr/0009-structuredcontent-is-what-the-agent-reads.md) and [ADR-0012](docs/adr/0012-the-text-block-is-a-summary.md). |
| **Summary line** | What `content[0].text` holds: one line describing the payload (`key=value`, `key[n]`, `key{n}`, long strings as `key=<bytes>B`), capped at 300 chars, produced by `summarize` unless the tool passes something better. The row formats (`tsv`, `columnar`, `bare`, `grouped`) are the exception — those render rows because the caller asked for a text view. |
| **Primary argument** | An argument a tool cannot run without. Declared `.optional()` in the schema and named in the config's `required: [...]`, so `defineTool` — not the SDK's Zod boundary — rejects the omission, as a readable `invalid_input` result instead of `MCP error -32602`. See [ADR-0013](docs/adr/0013-definetool-owns-required-arg-validation.md). |
| **Frame round-trip** | One JSON-RPC line leaving the server and re-parsing intact on the client side. What `src/transport.stdio.test.ts` asserts against a spawned bundle: any line that fails to re-parse is a *malformed frame*. |
